...
#if HAVE_TRILINOS && TRILINOS_HAVE_ZOLTAN2
...
//REMOVE DEAD CODE? VISUALIZEINPUT NEVER DECLARED
  ...
  if (*partmethod == 5 || *mappingmethod > 1){
    ...
    // Run Zoltan comparison Map Problem and store results in result_parts_copy
    zoltan_map_problem(nelem, xadj, adjncy, adjwgt, vwgt,
                       nparts, c_comm,
                       xcoord, ycoord, zcoord, coord_dimension,
                       &(result_parts_copy[0]), 
                       partmethod, mappingmethod);
    ...
    // Run Zoltan Mapping Problem and store results in result_parts
    zoltan_mapping_problem(nelem, xadj, adjncy, adjwgt, vwgt,
                           nparts, c_comm,
                           xcoord, ycoord, zcoord, coord_dimension,
                           result_parts, 
                           partmethod, mappingmethod);
    ...
    for (int i = 0; i < *nelem; ++i){
      if (result_parts[i] != result_parts_copy[i]){
        printf("i:%d result_parts:%d result_parts_copy:%d\n", i, 
               result_parts[i], result_parts_copy[i]);
        exit(1);
      }
    }
...
#else // TRILINOS NOT COMPILED WITH ZOLTAN2
  int mype2, size2;
  MPI_Comm_rank(c_comm, &mype2);
  MPI_Comm_size(c_comm, &size2);
...
#else // HOMME NOT COMPILED WITH TRILINOS
  int mype2, size2;
  MPI_Comm_rank(c_comm, &mype2);
  MPI_Comm_size(c_comm, &size2);
...
//REMOVE DEAD CODE? VISUALIZEOUTPUT NEVER DECLARED
...
#else // TRILINOS NOT COMPILED WITH ZOLTAN
  int mype2, size2;
  MPI_Comm_rank(c_comm, &mype2);
  MPI_Comm_size(c_comm, &size2);
...
void zoltanpart(
  ...
  zoltanpart_(nelem, xadj, adjncy, adjwgt, vwgt, 
              nparts, comm, 
              xcoord, ycoord, zcoord, coord_dimension, 
              result_parts,
              partmethod, mappingmethod);
...
void Z2PRINTMETRICS(
  ...
  z2printmetrics_(nelem, xadj, adjncy, adjwgt, vwgt, 
                  nparts, comm, result_parts);
}
